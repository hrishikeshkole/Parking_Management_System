...
Two_Wheel::Two_Wheel(std::string vname, std::string vnum){
    v_name = vname;
    v_num = vnum;
...
void Two_Wheel::Park_in_TwoWheel(){
    std::string time;
...
}//end Park_in_TwoWheel

void Two_Wheel::Park_out_TwoWheel(){
    std::string time;
    std::cout << std::setw(77) << "Time Guide <HH>space:space<MM>space<Time_Format> for eg. - 10 : 20 AM " << std::endl;
...
}//end Park_out_TwoWheel

void Two_Wheel::Parking_Charge(){
    const double TW_FARE {15.00}; // Two_Wheeler Parking Fare / Per Hour
    ...
    int hours = ComputeTime_TW(HH1,HH2,MM1,MM2,t1,t2) / 60;


    double charges = hours * TW_FARE;
...
std::string Two_Wheel::getvehicle_name(){ return v_name; }//end getvehical_name

std::string Two_Wheel::getvehicle_num(){ return v_num; }//end getvehical_num


int ConvertTimeToMinutes_TW(int hour, int min, bool pm){
    if (hour == 12)
        hour = 0;
    ...
    if (pm)
        calc += 12*60;
    return calc;
}//end ConvertTimeToMinutes_TW

// Compare times and return the absolute time difference in minutes
int ComputeTime_TW(int hour1, int hour2, int min1, int min2, bool pm1, bool pm2){
    int calc1 = ConvertTimeToMinutes_TW(hour1, min1, pm1);
    int calc2 = ConvertTimeToMinutes_TW(hour2, min2, pm2);
    if (calc2 > calc1)
    {
        return calc2 - calc1;
    }
