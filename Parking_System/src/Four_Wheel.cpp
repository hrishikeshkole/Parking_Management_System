...
Four_Wheel::Four_Wheel(std::string vname, std::string vnum){
    v_name = vname;
    v_num = vnum;
...
void Four_Wheel::Park_in_FourWheel(){
    std::string time;
...
}//end Park_in_FourWheel

void Four_Wheel::Park_out_FourWheel(){
    std::string time;
    std::cout << std::setw(77) << "Time Guide <HH>space:space<MM>space<Time_Format> for eg. - 10 : 20 AM " << std::endl;
...
}//end Park_out_FourWheel

void Four_Wheel::Parking_Charge(){
    const double FW_FARE {25.00};// Four_Wheeler Parking Fare / Per Hour
    ...
    int hours = ComputeTime_FW(HH1,HH2,MM1,MM2,t1,t2) / 60;


    double charges = hours * FW_FARE;
...
std::string Four_Wheel::getvehicle_name(){ return v_name; }//end getvehical_name

std::string Four_Wheel::getvehicle_num(){ return v_num; }//end getvehical_num


int ConvertTimeToMinutes_FW(int hour, int min, bool pm){
    if (hour == 12)
        hour = 0;
    ...
    if (pm)
        calc += 12*60;
    return calc;
}//end ConvertTimeToMinutes_FW

// Compare times and return the absolute time difference in minutes
int ComputeTime_FW(int hour1, int hour2, int min1, int min2, bool pm1, bool pm2){
    int calc1 = ConvertTimeToMinutes_FW(hour1, min1, pm1);
    int calc2 = ConvertTimeToMinutes_FW(hour2, min2, pm2);
    if (calc2 > calc1)
    {
        return calc2 - calc1;
    }
